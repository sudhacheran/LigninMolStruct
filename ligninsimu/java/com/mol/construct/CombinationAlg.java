...
class CombinationAlg { 
    ...
    private static ArrayList<ArrayList<Integer>> possiblegraph(ArrayList<ArrayList> arlist, int dp, int bc)
    {
    	...
    	//System.out.println(possiblerootNodes);
    	int i=0;
    	
    	for (List rootnode : possiblerootNodes)
	    {    		
	    	returnList.add((ArrayList<Integer>) rootnode);
	    	//System.out.println("rootnode="+rootnode);
	    	int prevCNode = (int) rootnode.get(1);
	    	//System.out.println(prevCNode);	
	    	int cnt =0;
	    	List<ArrayList<Integer>> brachedData = new ArrayList<ArrayList<Integer>>();
	    	for (List data : arlist)
		    {   		
	    		//System.out.println(prevCNode+"==" + (int)data.get(0));
	    		if (bc ==0 ||(brachedData.size() > 0 && (int)data.get(0) != (int)brachedData.get(0).get(1) && (int)data.get(1) != (int)brachedData.get(0).get(1)))
	    		{
	    		...
	    		if (bc != 0 & cnt==0 && (int)data.get(1) > (int)data.get(0)+1)
	    		{
    				...
    				//System.out.println("nextnode="+arlist.get(nc));
    				cnt++;
    ...
    public static void main (String[] args) { 
       ...
       // System.out.println(getCombination(n,0));
